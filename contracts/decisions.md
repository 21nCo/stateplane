...
| D5 | Draft 2020-12 JSON Schema bounded subset, reject unknown or malformed keyword values and breaking revisions; lengths count Unicode codepoints, and compatibility ignores only schema-node description annotations (not properties named `description`). Optional additions only after indexing readiness. No custom DSL whose translation might silently diverge. | STA-7 schema/compiler |
| D6 | Exact count/exists and record status from authority. Live keyset pages bind scope/query/schema and are not complete snapshots; optional sorts order missing, null, value ascending (reverse primary order descending), always ID ascending on ties. Export/long analysis uses pinned consistent boundary and fetch-time authorization. | STA-8 exact queries; STA-15 analytics; STA-17 export |
