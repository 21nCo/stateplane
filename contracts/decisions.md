...
The D3 target/envelope snapshot uses own data slots rather than ordinary-object assignment that an inherited setter could swallow. Key normalization, validity and string bounds scan actual UTF-16 code units through a trusted reader captured before caller input; later inherited `String.prototype.charCodeAt` or iterator changes cannot alter surrogate validation, scalar counts or White_Space trimming. A mutable `codePointAt` replacement would reopen the same issue. Authorization and exact committed replay retain their established precedence. STA-7 proves the corresponding adapter boundary, not this in-memory oracle alone.
