...
| D3 | Create-only and mandatory revision checks on existing records. Replace removes omitted optional fields; patch is explicit top-level set/unset. Null is present; missing is absent. Tombstones reserve keys. Link restrict/detach, no record cascades. Prevent lost updates and ambiguous resurrection. | STA-7 mutations |
| D4 | Receipt identity includes credential and operation in space; fingerprint includes precondition and canonical payload. Current authorization precedes receipt lookup, matching replay precedes stale-revision check. A lost response is safe to retry within documented retention; changed request is an error. Prevent duplicate effects without leaking revoked results. | STA-5 atomic store; STA-6 policy; STA-7 mutations; STA-9/10 transports |
