...
- The oracle captures its structured-clone operation before accepting caller input. Definition/revision schemas, mutation data and tombstones, returned records/query pages, and fresh/replayed receipt envelopes are separate copies: a later reassignment of the global clone function cannot alias caller values into authoritative state or expose mutable state through a response. Rejecting an invalid write still leaves rows, reservations, events, outbox and receipts unchanged; STA-7/8 must establish the same boundary in real adapters.
